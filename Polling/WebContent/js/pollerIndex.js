...
loadPolls($('#lg-menu').attr('currentSelectedCategory'),"false");
 ...
 $.post('CategoryLoadController',{}, function(responseJSON) {
	...
	$('body').on('click','#resetbutton',function(){
		...
		$.post('PasswordResetController', {email:$('#forgotemailID').val()}, function(responseJSON) {
	     
			console.log(responseJSON);
			
			$('#postforgotmodal').modal('show');
	...
	$('body').on('click', '.answerbox',function(){
		...
		$.post('AnswerLoadController', {id:poll_idJS,selectedOption:selectedoptionJS}, function(responseJSON) {
	...
	function loadPolls(Category,isCatClick)
	{
		 ...
		 $.post('IndexPollLoadController', {category:Category,loadedString:loadedpolls,isCatClick:isCatClick}, function(responseJSON) {
	         
			 if(responseJSON.Poll.length==0)
			{
		     ...
		     loadedpolls=responseJSON.loadedpolls;
		     $(window).data('ajaxready', true);
...
function appendPoll(pollId,title,description,imgSrc,taggedBy,option1,option2,option3,option4,fbCommentId,votes) {
	...
	poll=poll.concat(pollId);		
	poll=poll.concat('" data-layout="button_count" data-mobile-iframe="true"></div>');
